...
export default function UserChatInput({ sellerId, customerName }) {
  ...
  const [requestStatus, setRequestStatus] = useState('idle'); // idle | requested | accepted
  ...
  const fetchLatestChat = async () => {
    ...
    if (error) {
      console.error('❌ chat_logs 불러오기 실패:', error.message);
      return;
    }
  ...
  const handleSend = async () => {
    ...
    await supabase.from('chat_logs').insert([{
      ...
      messages: JSON.stringify(updatedLog), // 반드시 문자열 저장
      sentiment: null,
      ai_advice: advice
    }]);
  };
  ...
  useEffect(() => {
    const checkStatus = async () => {
      ...
      if (error) {
        console.error('❌ chat_requests 조회 실패:', error.message);
        return;
      }
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'chat_requests' }, payload => {
        ...
        if (status === 'requested') setRequestStatus('requested');
